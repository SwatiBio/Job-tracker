...
export const profile = createStore(async () => {
  ...
  if (!p) return null;
  // Parse JSON-in-string columns (temporary until WAYP-18 ships)
...
export const artifacts = createStore(async () => {
  ...
  return Array.isArray(data) ? parseArtifacts(data) : [];
});

export async function getArtifact(id) {
  const art = await api(`/artifacts/${id}`);
  return art ? parseArtifact(art) : null;
}
...
/** Parse variants JSON string into array (temporary until WAYP-18 ships). */
function parseArtifact(art) {
  if (art && typeof art.variants === 'string') {
    try { art.variants = JSON.parse(art.variants); } catch { art.variants = []; }
  }
  return art;
}

function parseArtifacts(list) {
  return list.map(parseArtifact);
}
